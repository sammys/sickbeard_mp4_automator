...
class ReadSettings:
    ...
    def __init__(self, directory, filename, logger=None):
        ...
        self.abitrate_copy = config.get(section, "audio-channel-bitrate-copy")
        try:
            self.abitrate_copy = int(self.abitrate_copy)
        except:
            self.abitrate_copy = 0
            log.warning("Audio copy bitrate was invalid, defaulting to use the source's per channel bitrate.")
        if self.abitrate_copy > 256:
            log.warning("Audio copy bitrates >256 may create errors with common codecs.")
